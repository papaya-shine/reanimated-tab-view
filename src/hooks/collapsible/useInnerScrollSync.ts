 ...
 */
export const useInnerScrollSync = (
  scrollRef: React.RefObject<Animated.ScrollView | Animated.FlatList<any>>
) => {
  const { outerScrollY, isHeaderCollapsed, headerHeight } = useCollapsibleContext();
    ...
    (currentOuterY) => {
      ...
      if (!isRouteFocused) {
        ...
        if (currentOuterY <= headerHeight) {
          ...
          scrollTo(scrollRef as any, 0, 0, false);
          scrollYSV.value = 0;
        }
      }
    },
  ...
  const handleScroll = useAnimatedScrollHandler({
    ...
    onEndDrag: (event: NativeScrollEvent) => {
      ...
      if (
        isRouteFocused &&
        lastDragY.value <= SCROLL_TRANSFER_THRESHOLD &&
        event.contentOffset.y < 0 && // Overscroll (iOS)
...
export const useIsInnerScrollEnabled = () => {
  const { isHeaderCollapsed } = useCollapsibleContext();
