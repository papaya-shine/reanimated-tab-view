...
import {
  REFRESH_TRIGGER_THRESHOLD,
  REFRESH_OPACITY_START_FACTOR,
  REFRESH_OPACITY_MID,
  REFRESH_DEFAULT_COLOR,
 ...
 */
export const RefreshIndicator = React.memo(() => {
  const { refreshOverscrollSV } = useHeaderContext();
  const { refreshing, refreshControlColor, onRefresh } = usePropsContext();
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    ...
    const opacity = interpolate(
      refreshOverscrollSV.value,
      [
        0,
        REFRESH_TRIGGER_THRESHOLD * REFRESH_OPACITY_START_FACTOR,
        REFRESH_TRIGGER_THRESHOLD,
      ],
      [0, REFRESH_OPACITY_MID, 1],
      Extrapolation.CLAMP
    );
