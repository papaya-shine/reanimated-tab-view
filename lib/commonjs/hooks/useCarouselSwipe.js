...
const ACTIVE_OFFSET_X = _reactNative.Platform.select({
  ios: [-10, 10],
  android: [-10, 10],
  // Lower = activates sooner (try 10-20)
  default: [-10, 10]
});

// Vertical distance that cancels the swipe (for scroll conflict on Android)
const FAIL_OFFSET_Y = _reactNative.Platform.select({
  ios: [-1000, 1000],
  // Effectively disabled on iOS
  android: [-20, 20],
  // Higher = more forgiving for diagonal swipes (try 15-30)
  default: [-1000, 1000]
});
const useCarouselSwipePanGesture = (updateCurrentRouteIndex, handleSwipeStart, handleSwipeEnd) => {
  ...
  const swipePanGesture = (0, _react.useMemo)(() => _reactNativeGestureHandler.Gesture.Pan().enabled(gestureEnabled).activeOffsetX(ACTIVE_OFFSET_X).failOffsetY(FAIL_OFFSET_Y).onStart(() => {
