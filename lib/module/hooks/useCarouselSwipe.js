...
const ACTIVE_OFFSET_X = Platform.select({
  ios: [-10, 10],
  android: [-10, 10],
...
const FAIL_OFFSET_Y = Platform.select({
  ios: [-1000, 1000],
...
export const useCarouselSwipePanGesture = (updateCurrentRouteIndex, handleSwipeStart, handleSwipeEnd) => {
  ...
  const swipePanGesture = useMemo(() => Gesture.Pan().enabled(gestureEnabled).activeOffsetX(ACTIVE_OFFSET_X).failOffsetY(FAIL_OFFSET_Y).onStart(() => {
