...
import { REFRESH_SCROLL_THRESHOLD, REFRESH_RESISTANCE_FACTOR } from '../../constants/refresh';
...
export const useScrollHandlers = ({
  ...
  const onScroll = useCallback(event => {
    ...
    if (gestureSourceSV.value === GestureSource.SCROLL) {
      ...
      if (event.contentOffset.y < 0 && canTriggerRefresh(onRefresh, refreshing, isRefreshTriggeredSV.value)) {
        // Track overscroll for refresh (with resistance)
        const overscroll = Math.abs(event.contentOffset.y) * REFRESH_RESISTANCE_FACTOR;
